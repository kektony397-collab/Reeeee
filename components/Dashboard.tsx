...
const Dashboard: React.FC = () => {
    ...
    // Ref to hold current ride data to stabilize stopRide callback
    ...
    }, [tripDistance, tripDuration, maxSpeed, userMileage, currentRide]);
    ...
    const stopRide = useCallback(async () => {
        ...
        if (tripDurationInterval.current) {
            clearInterval(tripDurationInterval.current);
            tripDurationInterval.current = null;
        }
        ...
        const { tripDistance, tripDuration, maxSpeed, userMileage, currentRide } = rideStateRef.current;
    ...
    useEffect(() => {
        // Cleanup on component unmount to stop an active ride
        return () => {
            if (isRiding) {
                stopRide();
            }
